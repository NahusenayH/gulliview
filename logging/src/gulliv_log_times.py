...
# Camera names from wall to door:  DOOR  3, 2, 1, 0  BACK WALL

input(f'GulliView timing script. Enter to begin.')
...
def flush_port():
     ...
     while True: 
        try:
            ...
            no_data += 1
            # if flushing has received no new data in latest x tries
...
model_str = "advanced" # Change to current model

filename = f'{log_timestamp}_{model_str}_1080_timing.csv'
with open(filename, 'w', newline='') as file:
    writer = csv.writer(file, dialect = 'excel')
    writer.writerow(['CameraID','ImportTime','TransformTime','SearchTime','Timestamp'])
...
while True: 
    sampleNo = 1
    latest_detection_cam = 100
    input(f"About to log timing data for model: {model_str} with {amountOfSamples} samples. Press any key to start.")
    flush_port()
    print("Running loop...")
    while True:
        try:
            while True: # Loop through all messages until the exception is thrown
                ...
                (import_time, transform_time, search_time, msecs, _, cam_name, length) = msg_header
                timestamp = msg_header[3]
                
                # print(import_time, transform_time, search_time, msecs)
                ...
                if (not detections) and (cam_name == latest_detection_cam):
                    ...
                    with open(filename, 'a', newline='') as file:
                        writer = csv.writer(file, dialect = 'excel')
                        writer.writerow([cam_name, import_time, transform_time, search_time, timestamp]) # log zero for det. status if nothing found
                    ...
                    for i in range(0, length):
                        ...
                        latest_detection_cam = camera_id
                        sampleNo += 1
                        print(f"sample: {sampleNo}")
                        with open(filename, 'a',newline='') as file:
                            writer = csv.writer(file, dialect = 'excel')
                            # Add 0s since other stamps use ns time || gammal kommentar, vad menas?
                            writer.writerow([cam_name, import_time, transform_time, search_time, timestamp])
